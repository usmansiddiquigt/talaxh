...
export default function HomeScreen({ navigation, route }) {
  ...
  const bannerTranslateY = scrollY.interpolate({
    ...
    outputRange: [0, -bannerH],
    extrapolate: 'clamp',
  });
  const bannerOpacity = scrollY.interpolate({
    inputRange:  [0, Math.max(1, bannerH * 0.6), Math.max(2, bannerH)],
    outputRange: [1, 0.5, 0],
    extrapolate: 'clamp',
  });
  const filterTranslateY = scrollY.interpolate({
    inputRange:  [0, Math.max(1, effectiveBannerH)],
    outputRange: [effectiveBannerH, 0],
    extrapolate: 'clamp',
  });
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      fetchFavorites();
      fetchUnreadNotif();
    }, [fetchListings, fetchFavorites, fetchUnreadNotif])
  );

  // Poll the unread notification count every 15s + on foreground,
  // so the bell badge updates without the user opening anything.
  useEffect(() => {
    if (!token) return;
    fetchUnreadNotif();
    const id = setInterval(fetchUnreadNotif, 15_000);
    const sub = AppState.addEventListener('change', (s) => {
      if (s === 'active') fetchUnreadNotif();
    });
    return () => {
      clearInterval(id);
      sub.remove();
    };
  }, [token, fetchUnreadNotif]);
    ...
    <SafeAreaView style={styles.safe}>
      ...
      {loading ? (
        ...
        <View style={{ flex: 1, overflow: 'hidden' }}>
          ...
          <Animated.View
            onLayout={e => setFilterH(e.nativeEvent.layout.height)}
            style={[
              styles.filterLayer,
          ...
          <Animated.ScrollView
            ...
            contentContainerStyle={{
              paddingTop: effectiveBannerH + filterH,
              paddingBottom: 110,
            }}
          ...
          >
            ...
            {/* Listing grid (rendered as rows of 2) */}
            {listings.length === 0 ? (
              <EmptyState
                icon="search"
              ...
              <View style={styles.grid}>
                {Array.from({ length: Math.ceil(listings.length / 2) }).map((_, rowIdx) => {
                  const a = listings[rowIdx * 2];
                  const b = listings[rowIdx * 2 + 1];
                  return (
                    <View key={rowIdx} style={styles.row}>
                      {a && renderItem({ item: a, index: rowIdx * 2 })}
                      {b && renderItem({ item: b, index: rowIdx * 2 + 1 })}
                    </View>
                  );
                })}
              </View>
