...
export async function fetchListings({
  category, search, minPrice, maxPrice, city, breed, gender,
...
export async function markNotificationRead(id) {
  ...
  if (error) console.warn('[api] markNotificationRead failed:', error.message);
}
...
export async function markAllNotificationsRead() {
  ...
  if (error) console.warn('[api] markAllNotificationsRead failed:', error.message);
}
...
// Number of conversations that contain unread inbound messages.
// Used for the Messages tab badge — matches the "Unread" filter count
// on the Messages screen (WhatsApp-style: one chat = one badge unit).
export async function fetchUnreadMessagesCount() {
  ...
  if (error) {
    console.warn('[api] fetchUnreadMessagesCount failed:', error.message);
    return 0;
  }
